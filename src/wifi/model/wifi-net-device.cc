...
void
WifiNetDevice::SetTopologyReceiveCallback (NetDevice::PromiscReceiveCallback cb)
{
  NS_LOG_FUNCTION (&cb);
  m_topologyRxCallback = cb;
}
...
// edit start @caiqiqi
    if (!m_topologyRxCallback.IsNull ())
    {
      // FIXME it is necessary to copy the packet?
      m_topologyRxCallback (this, packet->Copy (), llc.GetType (), from, to, type);
      return;
    }
